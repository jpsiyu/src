...
-include("common.hrl").
-include("scene.hrl").
%-define(LENGTH, 10).                        %% 格子的长度，
%-define(WIDTH, 15).                         %% 格子的宽度
%-define(MAP_LENGTH, 20).                    %% 默认20 可支持长度小于等于像素为60 * ?LENGTH * 20 的的地图 即长度为12000像素的地图 宽度不限
-define(LENGTH, 8).
-define(WIDTH, 9).
-define(MAP_LENGTH, 25). 

-define(MAP_LENGTH_M, 0).                   % 九宫格中间格子编号
-define(MAP_LENGTH_R, 1).                   % 九宫格右边格子编号
-define(MAP_LENGTH_L, -1).                  % 九宫格左边格子编号 
-define(MAP_LENGTH_U, -?MAP_LENGTH).        % 九宫格上边格子编号
-define(MAP_LENGTH_D, ?MAP_LENGTH).         % 九宫格下边格子编号
-define(MAP_LENGTH_LU, -(?MAP_LENGTH+1)).   % 九宫格左上边格子编号
-define(MAP_LENGTH_LD, (?MAP_LENGTH-1)).    % 九宫格左下边格子编号
-define(MAP_LENGTH_RU, -(?MAP_LENGTH-1)).   % 九宫格右上边格子编号
-define(MAP_LENGTH_RD, ?MAP_LENGTH+1).      % 九宫格右下边格子编号
...
move_broadcast(Q, CopyId, X1, Y1, X2, Y2, BinData, BinData1, BinData2, SendList) ->
    ...
    case SendList =/= [] of
        true ->
            mod_mon_agent:apply_cast(Q, lib_scene_calc, move_mon_broadcast, [CopyId, XY1, XY2, SendList]);
        false ->
            skip
    end.
...
%    %% 目标位置全部九宫格格子编号
%    AllArea1 = [XY1, XY1 + ?MAP_LENGTH_LU, XY1 + ?MAP_LENGTH_U, XY1 + ?MAP_LENGTH_RU, XY1 + ?MAP_LENGTH_L, XY1 + ?MAP_LENGTH_R, XY1 + ?MAP_LENGTH_LD, XY1 + ?MAP_LENGTH_D, XY1 + ?MAP_LENGTH_RD],
%    %% 原始位置全部九宫格格子编号
%    AllArea2 = [XY2, XY2 + ?MAP_LENGTH_LU, XY2 + ?MAP_LENGTH_U, XY2 + ?MAP_LENGTH_RU, XY2 + ?MAP_LENGTH_L, XY2 + ?MAP_LENGTH_R, XY2 + ?MAP_LENGTH_LD, XY2 + ?MAP_LENGTH_D, XY2 + ?MAP_LENGTH_RD],
...
%                false ->{[E|AddUserL], DelUserL -- [E], MoveUserL}
%            end
%    end,
