...
-export([
            ...
            get_scene_user_for_battle/5,
            get_line_user_for_battle/9,
...
get_scene_user_for_battle(CopyId, X, Y, Area, Group) ->
    ...
    Y1 = Y + Area,
    Y2 = Y - Area,
    [ User || User <-AllUser, User#ets_scene_user.x >= X2 andalso User#ets_scene_user.x =< X1, User#ets_scene_user.y >= Y2 andalso User#ets_scene_user.y =< Y1, User#ets_scene_user.hp > 0, (Group == 0 orelse User#ets_scene_user.group /= Group), User#ets_scene_user.visible == 0].

%% 直线攻击目标
%% OX,OY 目标点坐标
%% X,Y   攻击者坐标
get_line_user_for_battle(CopyId, OX, OY, X, Y, Area, K, B, Group) -> 
     AllArea = lib_scene_calc:get_the_area(X, Y),
    AllUser = get_all_area_user(AllArea, CopyId),
    X1 = X + Area,
    X2 = X - Area,
    ...
    [ User || User <-AllUser, 
        ...
        (Group == 0 orelse User#ets_scene_user.group /= Group), User#ets_scene_user.visible == 0, F(User#ets_scene_user.x,  User#ets_scene_user.y)].

get_scene_user_for_assist(CopyId, X, Y, Area, TeamId, KfTeamId, Group, Sign) ->
    AllArea = lib_scene_calc:get_the_area(X, Y),
    AllUser = get_all_area_user(AllArea, CopyId),
    X1 = X + Area,
    X2 = X - Area,
    Y1 = Y + Area,
    Y2 = Y - Area,
    [User || User <-AllUser, User#ets_scene_user.hp > 0, (Group == 0 orelse User#ets_scene_user.group == Group), User#ets_scene_user.x >= X2 andalso User#ets_scene_user.x =< X1, User#ets_scene_user.y >= Y2 andalso User#ets_scene_user.y =< Y1, ((User#ets_scene_user.pid_team =:= TeamId andalso TeamId /= 0) orelse (User#ets_scene_user.kf_teamid == KfTeamId andalso KfTeamId /= 0) orelse User#ets_scene_user.scene == 109 orelse Sign == 1)].
...
%% 获取场景中的1v1数据
get_user_1v1(_CopyId, Id, PlatFrom, ServerNum) -> 
    case get([Id, PlatFrom, ServerNum]) of 
        undefined -> {0,0,1,0};
        User -> 
            BA = User#ets_scene_user.battle_attr, 
            {BA#battle_attr.combat_power, User#ets_scene_user.hp, User#ets_scene_user.hp_lim, User#ets_scene_user.scene}
    end.

%% 怪物追踪目标时获取目标信息
get_att_target_info_by_id([MonAid, Key, AttType, MonGroup]) -> 
    AttInfo = case get_user(Key) of
        Player when is_record(Player, ets_scene_user), Player#ets_scene_user.hp > 0 -> 
