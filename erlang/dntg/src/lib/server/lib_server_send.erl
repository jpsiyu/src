...
send_to_all(MinLv, MaxLv, Bin) ->
    mod_disperse:cast_to_unite(lib_unite_send, send_to_all, [MinLv, MaxLv, Bin]).
...
rand_to_process_by_id(Id) ->
    %TT = case get("lib_server_send_rand") of
    %    undefined->
    %        0;
    %    _TT ->
    %        case  _TT > 1000000 of
    ...
    %        end
    %end,
    %put("lib_server_send_rand", TT+1),
    %Rand = TT rem ?SERVER_SEND_MSG + 1,
    %misc:whereis_name(global, misc:player_send_process_name(Id, random:uniform(?SERVER_SEND_MSG))).
    misc:whereis_name(global, misc:player_send_process_name(Id, 1)).
