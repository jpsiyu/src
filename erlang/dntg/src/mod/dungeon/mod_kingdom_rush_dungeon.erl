%%------------------------------------------------------------------------------
%% @Module  : mod_kingdom_rush_dungeon
%% @Author  : liangjianxiong
%% @Email   : ljianxiong@qq.com
%% @Created : 2012.10.17
%% @Description: 皇家守卫军塔防副本服务器
%%------------------------------------------------------------------------------
...
handle_info('king_dun_create_mon', State) ->
	...
	Level2 = Level + 1,
	KingDunData1 = data_king_dun:get(Level),
	KingDunData2 = data_king_dun:get(Level2),
	MonName = KingDunData1#king_dun_data.mon_name,
	KingDunState = lib_kingdom_rush_dungeon:get_king_state(),
    ...
    %1.保存塔防副本状态.
    lib_kingdom_rush_dungeon:save_king_state([{enemy_mon_list, NewEnemyMonList},
            {last_begin_time, util:unixtime()},
            {create_mon_timer, CreateMonTimer}]),

    %1.召唤怪物.
    CreateData = [Level, MaxLevel, SceneId, OwnerId],
    mod_scene_agent:apply_cast(SceneId, lib_kingdom_rush_dungeon, create_mon, [?CREATE_ENEMY_MON, CopyId, CreateData]),
	...
	{Result, Level2} = 
		case KingDunState#king_dun_state.is_start of
			...
			false ->
				...
				%3.保存副本数据.
				lib_kingdom_rush_dungeon:save_king_state([{is_start, true},
														  {last_begin_time, util:unixtime()},
														  {create_mon_timer, CreateMonTimer}]),
				{1, Level}
		end,
	...
	MonName =
		case Level == 1 of
			...
			false ->
				...
				KingDunData2 = data_king_dun:get(Level-1),
				KingDunData2#king_dun_data.mon_name
		end,
	
	%2.获取时间.
	Time1 = KingDunData#king_dun_data.time,
	KindDunState = lib_kingdom_rush_dungeon:get_king_state(),
...
handle_info('king_dun_get_score', State) ->
	lib_kingdom_rush_dungeon:send_score(State),
	{noreply, State};
...
handle_info({'king_dun_upgrade_building', Data}, State) ->
			...
			true ->
				...
				case lists:keyfind(MonAutoId, 2, BuildingList) of
					...
					BuildingData ->
						...
						LastMid = data_king_dun_config:get_building_last_level(NextMonMid),
	...
	case Result of
		...
		1 ->
			...
			SoldierAutoIdList = lib_kingdom_rush_dungeon:get_soldier_auto_id(SoldierList, []),
			...
			lib_kingdom_rush_dungeon:save_king_state([{building_list, BuildingList2},
													  {score, Score2}]),
			lib_kingdom_rush_dungeon:send_score(State),

			%3.发送升级建筑到场景服务器.
            CreateData = [NextMonMid, KillMonList, SceneId, Position],
            mod_scene_agent:apply_cast(SceneId, lib_kingdom_rush_dungeon, create_mon, [?UPGRADE_BUILDING, CopyId, CreateData])
	end,

	%3.发送结果.
    {ok, BinData} = pt_613:write(61304, [Result]),
	RoleList = State#dungeon_state.role_list,
    [lib_server_send:send_to_uid(Role#dungeon_player.id, BinData) || Role <- RoleList],	
	{noreply, State};
...
handle_info({'king_dun_upgrade_skill', Data}, State) ->
	[MonAutoId, SkillId, SkillLevel] = Data,
	CopyId = self(),
	SceneId = State#dungeon_state.begin_sid,
	KingDunState = lib_kingdom_rush_dungeon:get_king_state(),
	BuildingList = KingDunState#king_dun_state.building_list,
	Score = KingDunState#king_dun_state.score,
	{_SkillId, _Pro, Score1} = data_king_dun_config:get_skill({SkillId, SkillLevel}),
	Score2 = Score - Score1, 
	...
	case Result of
		...
		1 ->
            ...
            case SoldierList == [] of
                ...
                %2.升级士兵.
                false ->			
                    lib_kingdom_rush_dungeon:get_soldier_auto_id(SoldierList, [])
            end,

            %3.发送升级技能到场景服务器.
            CreateData = [MonIdList, SkillList2],
            mod_scene_agent:apply_cast(SceneId, lib_kingdom_rush_dungeon, create_mon, [?UPGRADE_SKILL, CopyId, CreateData])
    end,
	
    {ok, BinData} = pt_613:write(61305, [Result, MonAutoId, SkillId]),
	RoleList = State#dungeon_state.role_list,
    [lib_server_send:send_to_uid(Role#dungeon_player.id, BinData) || Role <- RoleList],	
	{noreply, State};
...
handle_info({'king_dun_set_building_info', BuildingData}, State) ->
	...
	lib_kingdom_rush_dungeon:save_king_state([{building_list, BuildingList}]),		
	{noreply, State};
...
handle_info({'king_dun_add_score', Score}, State) ->
	...
	%2.保存积分.
	lib_kingdom_rush_dungeon:save_king_state([{add_score, Score1}]),
	%3.发送客户端.
	lib_kingdom_rush_dungeon:send_score(State),
	{noreply, State};
...
handle_info('king_dun_load_max_level', State) ->
	lib_kingdom_rush_dungeon:load_max_level(State),
	{noreply, State}.
