%%%-----------------------------------
%%% @Module  : timer_unite_midday12_frame
%%% @Author  : huangcha
%%% @QQ   : 215837829
%%% @Created : 2011.11.26
%%% @Description: 后台定时服务框架(每天中午十二点整点执行一次)
%%%-----------------------------------
-module(timer_unite_midday12_frame).
-include("common.hrl").
...
get_fist_time() ->
    ...
    Now      = util:unixtime(),
    Midday   = util:unixdate() + 86400 div 2,
    case Now < Midday of
        true ->
            Midday - ZeroTime -(Now - ZeroTime);
        _ ->
            %十五分钟之内的立即启动答题
            NextTime = 86400 - (Now - Midday),
