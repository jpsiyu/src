%%%-----------------------------------
%%% @Module  : timer_unite_shengxiao20_frame
%%% @Author  : guoxi
%%% @Email   : guoxi@jieyou.cn
%%% @Created : 2012.5.30
%%% @Description: 后台定时服务框架(每天晚上20时执行一次，周二和周四启动生肖大奖活动)
%%%-----------------------------------
...
% 回调模块列表
-define(MODLE_LIST, [timer_shengxiao]).
%%-define(MODLE_LIST, []).
% 休眠间隔
...
get_fist_time() ->
    ...
    StartTime   = util:unixdate() + Hour * 60 * 60 + Min * 60,
	%io:format("Now:~p, StartTime - ?SHENGXIAO_BROAD:~p~n", [Now, StartTime - ?SHENGXIAO_BROAD]),
    case Now < StartTime - ?SHENGXIAO_BROAD of
        true ->
            StartTime - ZeroTime - (Now - ZeroTime);
        _ ->
            %% 活动时间内直接打开
            NextTime = 86400 - (Now - StartTime),
            case 86400 - NextTime < ?SHENGXIAO_LONG - ?SHENGXIAO_BROAD of
                true ->
					%% 0秒时开始活动
					60 - (Now rem 60);
                _ ->
