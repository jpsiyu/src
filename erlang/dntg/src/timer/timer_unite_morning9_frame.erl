%%%-----------------------------------
%%% @Module  : timer_unite_morning9_frame
%%% @Author  : hekai
%%% @Description: 后台定时服务框架(每天早上九点整点执行一次)
%%%-----------------------------------
-module(timer_unite_morning9_frame).
-include("common.hrl").
...
% 回调模块列表
-define(MODLE_LIST, [timer_festival_card, timer_off_line]).
...
get_fist_time() ->
    ...
    Now      = util:unixtime(),
    Morning = util:unixdate() + 32400,
%%	Morning = util:unixdate() + 14400,
    case Now < Morning of
        true ->
            Morning - ZeroTime -(Now - ZeroTime);
        _ ->
            NextTime = 86400 - (Now - Morning),
